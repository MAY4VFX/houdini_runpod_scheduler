...
def build_topnet(karma):
    ...
    # These two used to be created UNLOCKED: their OnCreated called
    # allowEditingOfContents() to re-assert the internal scheduler override
    # (Ruling R22). An unlocked HDA instance saves its whole internal network
    # into the .hip, so the fixture carried a frozen copy of whatever generate
    # script was installed the day it was built and went on using it however
    # many times the HDA was rebuilt -- three live runs downloaded nothing
    # for exactly that reason. Ruling R53 removed the unlock: the override is
    # baked into the definition and the instances are locked like any other
    # asset. This stays as a belt for a fixture regenerated with an older
    # installed asset, and _verify() asserts it stuck.
