...
def compute(node):
    ...
    ``ls`` on the sync pod (``None`` if unreachable); ``monthly_cost`` is
    that project's share of ``vol_total``, prorated by size and by the
...
Summary:
    ...
    cleanup candidates -- projects with no cook in over 30 days, each with
    its real on-disk size (Task 12's housekeeping `ls`, via the sync pod)
    and an estimated $/month: that project's byte share of the period's
    actual `GET /billing/networkvolumes` charge, prorated to 30 days (Use
    Billing only -- otherwise shown as "n/a", never a guessed $/GB rate).
