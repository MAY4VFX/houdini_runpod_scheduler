...
- **Живость пода измеряется по всем каналам, а не только по HTTP (Ruling R34).**
  ...
  **Не проверено вживую:** держит ли сама RunPod постоянное SSH-соединение к
  поду. Если да, `ssh_sessions` всегда ≥1 и `kill --all` перестанет гасить
  что-либо само — отказ в безопасную сторону, но требующий `--even-if-running`.
  Проверяется одной командой на живом поде, когда поды снова можно поднимать.
