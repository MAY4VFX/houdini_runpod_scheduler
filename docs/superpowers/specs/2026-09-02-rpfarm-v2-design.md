...
└───────────────────────────────┘            │           │ /workspace           │
                                             ...
                                             │  hserver → lic.example.invalid:1715   │
                                             └──────────────────────────────────┘
```

Домашняя инфраструктура участвует только лицензиями: `lic.example.invalid:1715` →
...
- **Тайминги живого пода** (CPU, `HOUDINI_VERSION=.`, volume `2ze7qdwkt3`):
  ...
  `hserver -S lic.example.invalid:1715` отработал (`Successfully changed server
  listing.`), `hserver -l` показал `Connected To: http://lic.example.invalid:1715`
...
- Лицензии: `doctor` проверяет `lic.example.invalid:1715`; задача с ошибкой лицензии в логе
...
1. Удалить с volume `/workspace/houdini` (20.5.684), поставить 22.0.393 пресетом upload
   (тарболл с workstation01 `/home/may/Downloads/houdini-22.0.393-linux_x86_64_gcc14.2.tar.gz`
   или с Mac). Скачивание с sidefx.com с workstation01 не использовать (SNI-фильтрация,
...
3. Dokploy: удалить проект `runpodfarm` (сервер + Redis). Oracle: снять socat 6381,
   оставить 1715. Домен `redis.example.invalid` и `db.example.invalid` — убрать.
