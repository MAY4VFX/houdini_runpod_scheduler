...
- **Живость пода измеряется по всем каналам, а не только по HTTP (Ruling R34).**
  ...
  * `ssh_sessions` при этом честно показывал 0, и это была правда:
  ...
  Отсюда правило: **мгновенный замер — не доказательство**. Воркер сэмплирует
  оба признака раз в 2 с фоновым потоком и отдаёт `busy_idle_s` — сколько
  прошло с момента, когда хоть что-то выглядело живым. Судить нужно по нему;
  `transfers` и `ssh_sessions` остаются сырыми фактами для диагностики.
