...
- **Зависимости сцены — это ДВА разных вопроса, и одним API они не решаются
  ...
  |---|---|---|---|
  | как было до 2026-09-05 | 116 | 804 | 10.0 ГБ |
  | R38 (ветвь + пармы вывода) | 17 | 78 | 1.2 ГБ |
  | **R39 (пармы + USD)** | **156** | **155** | **2.8 ГБ** |
  ...
  * **Штатное окно вместо своего — для парм-ссылок.**
    `hou.ui.displayFileDependencyDialog` — то самое `File > Pre-Flight Scene`;
    им пользуется сам SideFX (`cloud.py:_onShowFileReferenceDialog`,
    `hqrop.py:copyProjectFilesToSharedFolder`), вызываем позиционно, как они.
    Пармы вывода уходят туда как `forced_unselected_patterns`: артист ВИДИТ
    снятую галочку и может её вернуть, вместо тихого выбрасывания. Ответ окна
    берём как есть — если он вернул галочку на `outputimage`, это его решение.
  * **Состояние выбора у Houdini своё.** `hou.fileReferences(include_all_refs=False)`
    отдаёт только отмеченное; в батче читаем именно его — тем же вызовом, каким
    SideFX обходятся, когда их окно пропущено (`hqrop.getSelectedFileReferences`).
    Поэтому `rpfarm_exclude` на ноде **сузился до USD-файлов**: для парм-ссылок
    источник правды теперь один, и он не наш. Проверено: записать это состояние
    из HOM нечем (`hou.Parm.setSelect`/`setAutoSelect` — это выделение канала в
    анимационном редакторе, к `include_all_refs` отношения не имеет), пишет его
    только само окно; в нетронутой сцене `False` возвращает всё (115 из 115).
  * **Честный предел штатного окна, который пришлось решать.** Оно умеет только
    пары `(Parm, pattern)`. У файла, который USD-слой называет ИЗНУТРИ себя,
    парма нет — ни строки в `hou.fileReferences()`, ни строки в диалоге, ни API,
    чтобы её добавить. Поэтому USD-половина показывается ВТОРЫМ окном —
    нашим (`rpfarm/preflight.py`, оно же и осталось единственной причиной этому
    модулю жить). Два окна подряд — цена за то, что 1.6 ГБ исходных текстур
    можно снять в два клика, а не «или всё, или ничего».
  ...
  * **UDIM и относительные пути.** У шаблона `<UDIM>` `resolvedPath` ПУСТОЙ
