...
Лицензии — свой sesinetd: `sesinetd_host`/`sesinetd_port` из
`~/.rpfarm/config.toml`. **Дефолта в коде нет и быть не должно**: адрес
лицензионного сервера принадлежит его владельцу, а не проекту, и в
отслеживаемые файлы не пишется (см. правило про домашние адреса ниже).
У этой фермы цепочка такая: публичный релей → socat-юнит `lic-proxy.service`
→ домашний лицензионный сервер; конкретные адреса — в
